...
function getStoredThreadId(): string {
  const stored = localStorage.getItem("deepsearch_thread_id")
  return stored || generateThreadId()
}

function storeThreadId(id: string): void {
  localStorage.setItem("deepsearch_thread_id", id)
}
  ...
  useEffect(() => {
    ...
    function connect() {
      ...
      socket.onmessage = (event) => {
        ...
        try {
          const payload = JSON.parse(event.data) as SocketMessage & MonitorMessage
